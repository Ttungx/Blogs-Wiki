...
> 状态：**方案 A/B/C 代码已全部落地**（worktree `feat-d1-write-budget`，尚未合并/部署；本地四道门禁全绿：update 71+smoke、d1 60、worker 93、check:worker 干净）。合并部署并跑完 §9 检查单前，**cron 保持空**——2026-09-02 已紧急置空并 deploy，禁止只把 `crons` 改回来。
