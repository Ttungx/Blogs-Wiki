...
| `openai-developers` | 已适配 | [OpenAI Developers](https://developers.openai.com/blog) | https://developers.openai.com/blog | 开发者平台 / API / Codex / Agents | Listing（`/blog` 单页 29 篇，article_paths=/blog/ 白名单，exclude /blog/topic） | 无（页面无 hreflang） | https://developers.openai.com/blog | 2026-09-12 接入：RSS 是全站资源聚合（52 条 platform 文档 + 27 条视频/仓库，无 blog 文章）不可用作发现；文章页无任何机器可读日期且 og 元数据混乱 → `date_fallback: conservative`（显示"无"）；`/blog/intro` 着陆页 262 词被长度门禁拒。已转 active，28 篇本地回填+同步 |
