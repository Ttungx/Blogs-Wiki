...
| `qwen` | 已适配（已迁移 qwen.ai） | [Qwen](https://qwen.ai/research) | https://qwen.ai/research | LLM / 多模态 / Agent | API（`/api/v2/article/retrieval`，GET+list_method，37 篇全量带正文） | 新站 zh 原生（en 空，无需翻译直通） | https://qwen.ai/research | 2026-09-13 迁移：新站为纯 SPA（无 RSS/sitemap/静态链接，详情页软 404），旧站 qwenlm.github.io 冻结于 2025-09 且未重定向；api 模式为此泛化 content_in_list/list_method/extra.date/path-slug（内容内嵌列表，HTML turndown 转换）；新旧 slug 零重叠，旧 44 篇存档保留，新 20 篇已入库其余增量补 |
| `openai-developers` | 已适配 | [OpenAI Developers](https://developers.openai.com/blog) | https://developers.openai.com/blog | 开发者平台 / API / Codex / Agents | Listing（`/blog` 单页 29 篇，article_paths=/blog/ 白名单，exclude /blog/topic） | 无（页面无 hreflang） | https://developers.openai.com/blog | 2026-09-12 接入：RSS 是全站资源聚合（52 条 platform 文档 + 27 条视频/仓库，无 blog 文章）不可用作发现；文章页无机器可读日期（无 meta/JSON-LD），发表日在文章自身 <header> 的可见文本里——2026-09-13 复核每页恰一个日期且与列表排序一致，已去 conservative 改走 h1/header 可见日期启发式（resolveHeadingDate 增加逐 header 兜底），存量 28 篇日期已修复为真实发表日；`/blog/intro` 着陆页 262 词被长度门禁拒。已转 active，28 篇本地回填+同步 |
