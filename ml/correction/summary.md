...
### 模式 9：zh-cn 演示翻译存根（w04 发现，已处置）

**特征**：demo 占位是 shadow 目录的 zh-cn 演示翻译存根（`translation_model: "GPT-5（演示内容）"`，1.7-2KB，共 3 篇）。
**后续验证：Gold 语料 bw-c-001 正文为真实全文，无需改标**；demo 只存在于 zh-cn 演示存根。
**处置（用户指令，2026-08-30 执行）**：D1 article_versions 删除 3 行演示翻译（三篇均保留 en 原文版）；工作文件同名 3 个删除。
