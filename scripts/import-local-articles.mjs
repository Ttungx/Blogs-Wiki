...
const OUT_SQL = join(ROOT, '.tmp-import-articles.sql');
...
for (const [articleId, versions_] of groups) {
  ...
  sql.push(
    ...
    `${esc(isoDate(fm.published_at))}, ${esc(fm.image_url)}, ${esc(fm.author)}, ${esc(fm.source_domain)}, datetime('now'), datetime('now')) ` +
  ...
  for (const v of versions_) {
    sql.push(
      `INSERT INTO article_versions (article_id, language, title, content_markdown, excerpt, provenance, translation_model, original_alt_url, updated_at) ` +
      `VALUES (${esc(articleId)}, ${esc(v.lang)}, ${esc(v.fm.title)}, ${esc(v.body)}, ` +
      `${esc(v.fm.excerpt)}, ${esc(v.fm.provenance ?? 'model')}, ${esc(v.fm.translation_model)}, ${esc(v.fm.original_alt_url)}, datetime('now')) ` +
      `ON CONFLICT(article_id, language) DO UPDATE SET title=excluded.title, content_markdown=excluded.content_markdown, ` +
      `excerpt=excluded.excerpt, provenance=excluded.provenance, translation_model=excluded.translation_model, updated_at=datetime('now');`
    );
    versions++;
